 ...
 *
//www.ietf.org/rfc/rfc4648.txt>.
 *
 * Be careful with error checking.  Here is how you would typically

 *
 * bool ok = shbase32_decode_alloc (in, inlen, &out, &outlen);
 * if (!ok)
 ...
 * if (out == NULL)
 ...
 data in OUT/OUTLEN
 *
 * size_t outlen = shbase32_encode_alloc (in, inlen, &out);
...
struct shbase32_decode_context
{
  unsigned int i;
  char buf[8];
};
...
size_t shbase32_encode_alloc (const char *in, size_t inlen, char **out)
{
  ...
  *out = malloc (outlen);
  ...
  shbase32_encode (in, inlen, *out, outlen);
...
/* Initialize decode-context buffer, CTX.  */
  void
shbase32_decode_ctx_init (struct shbase32_decode_context *ctx)
{
  ctx->i = 0;
}
...
static char * get_8 (struct shbase32_decode_context *ctx, char const **in, char const *in_end, size_t *n_non_newline)
{
 ...
 *OUTLEN holds the length of decoded bytes in OUT.  Note that as soon
 ...
 Initially, CTX must have been initialized via shbase32_decode_ctx_init.
...
bool shbase32_decode_ctx(struct shbase32_decode_context *ctx, const char *in, size_t inlen, char *out, size_t *outlen)
{
 ...
 *OUT and *OUTLEN parameters to differentiate between successful
 ...
 undefined. */
bool shbase32_decode_alloc_ctx (struct shbase32_decode_context *ctx, const char *in, size_t inlen, char **out, size_t *outlen)
{
  ...
  *out = malloc (needlen);
  ...
  if (!shbase32_decode_ctx (ctx, in, inlen, *out, &needlen))
  {
    free (*out);
    *out = NULL;
    return FALSE;
...
{
  return ((int)shbase32_decode_ctx(NULL, in, in_len, out, out_len_p));
}

int shbase32_adecode(char *in, size_t in_len, unsigned char *out, size_t *out_len_p)
{
  return ((int)shbase32_decode_alloc_ctx(NULL, in, in_len, out, out_len_p));
}
...
_TEST(shmem_base32)
{
  ...
  strcpy(data, "ABC234DEF567ABC2");

  memset(secret, 0, sizeof(secret));
  secret_len = sizeof(secret);
  _TRUE(shbase32_decode(data, strlen(data), secret, &secret_len));

  dec_len = sizeof(dec_data);
  memset(dec_data, 0, sizeof(dec_data));
  shbase32_encode(secret, secret_len, dec_data, dec_len);

  _TRUE(0 == strcmp(dec_data, data));
}
