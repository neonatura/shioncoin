...
#ifndef __SERVER__TXFEERATE_H__
#define __SERVER__TXFEERATE_H__
...
{
private:
    CAmount nCoinPerK; // unit is satoshis-per-1,000-bytes
...
public:
    ...
    explicit CFeeRate(const CAmount& _nCoinPerK) 
    {
      nCoinPerK = _nCoinPerK;
    }
    ...
    friend bool operator<(const CFeeRate& a, const CFeeRate& b) { return a.nCoinPerK < b.nCoinPerK; }
    friend bool operator>(const CFeeRate& a, const CFeeRate& b) { return a.nCoinPerK > b.nCoinPerK; }
    friend bool operator==(const CFeeRate& a, const CFeeRate& b) { return a.nCoinPerK == b.nCoinPerK; }
    friend bool operator<=(const CFeeRate& a, const CFeeRate& b) { return a.nCoinPerK <= b.nCoinPerK; }
    friend bool operator>=(const CFeeRate& a, const CFeeRate& b) { return a.nCoinPerK >= b.nCoinPerK; }
    CFeeRate& operator+=(const CFeeRate& a) { nCoinPerK += a.nCoinPerK; return *this; }
...
#define INF_FEERATE(_iface) (double)(MAX_MONEY(_iface))
static const double MIN_PRIORITY = 10;
static const double MAX_PRIORITY = 1e16;
#define INF_PRIORITY(_iface) (1e9 * INF_FEERATE(_iface))
...
public:
    ...
    CBlockPolicyEstimator(int ifaceIndexIn, const CFeeRate& minRelayFee);
    ...
    void processBlockTx(unsigned int nBlockHeight, CPoolTx& entry);

    /** Process a transaction accepted to the mempool*/
    void processTransaction(CPoolTx& entry, bool fCurrentEstimate);
...
#endif /* ndef __SERVER__TXFEERATE_H__ */
