...
int GetExecTxMode(CTransaction& tx)
{
	uint160 hashExec;
	int nMode;
	int nOut;
  ...
  if (nOut == -1)
	...
	nMode = -1;
  if (!DecodeExecHash(tx.vout[nOut].scriptPubKey, nMode, hashExec)) {
    return (-1);
  }
...
int CExec::VerifyTransaction()
{
	time_t now;
	int err;

	err = CExecCore::VerifyTransaction();
	if (err)
		return (err);
...
int64 CExec::CalculateFee(CIface *iface, int nHeight, int nContentSize, time_t nLifespan)
{
	if (nContentSize == -1) {
		nContentSize = GetContentSize();
	}
	return (GetExecOpFee(iface, nHeight, nContentSize));
}

int64 CExecCall::CalculateFee(CIface *iface, int nHeight, int nContentSize, time_t nLifespan)
{
...
int CExecCall::VerifyTransaction()
{
	int err;
