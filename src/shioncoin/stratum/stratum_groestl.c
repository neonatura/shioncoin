 ...
 */
static void stratum_groestl_close(int fd, struct sockaddr *net_addr)
{
	stratum_close(fd, net_addr);
}

static uint64_t pend_block_height[MAX_COIN_IFACE];
static int is_stratum_groestl_task_pending(int *ret_iface)
{
  ...
  for (ifaceIndex = 1; ifaceIndex < MAX_COIN_IFACE; ifaceIndex++) {
		if (!is_stratum_miner_algo(ifaceIndex, ALGO_GROESTL))
			continue;
...
static void stratum_groestl_task_gen(task_attr_t *attr)
{
  task_t *task;
  scrypt_peer peer;
...
static void stratum_groestl_timer(void)
{
  ...
  blk_iface = 0;
  is_new = is_stratum_groestl_task_pending(&blk_iface);
...
static void stratum_groestl_term(void)
{

  unet_unbind(UNET_STRATUM_GROESTL); /* close listening socket. */

}

static void stratum_groestl_accept(int fd, struct sockaddr *net_addr)
{
  sa_family_t in_fam;
	user_t *user;
  char buf[256];
  ...
  if (in_fam == AF_INET) {
    ...
    sprintf(buf, "stratum_accept: received connection (%s port %d).", inet_ntoa(addr->sin_addr), get_stratum_port(ALGO_GROESTL));
    shcoind_log(buf);  
  } else {
...
int stratum_groestl_init(void)
{
	...
	err = unet_bind(UNET_STRATUM_GROESTL, get_stratum_port(ALGO_GROESTL), NULL);
	if (err)
		return (err);

	unet_timer_set(UNET_STRATUM_GROESTL, stratum_groestl_timer); /* x1/s */
	unet_connop_set(UNET_STRATUM_GROESTL, stratum_groestl_accept);
	unet_disconnop_set(UNET_STRATUM_GROESTL, stratum_groestl_close);
