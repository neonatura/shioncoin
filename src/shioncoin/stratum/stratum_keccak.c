...
#define KECCAK_WORK_CYCLE_TIME 720 /* 12min */
 ...
 */
static void stratum_keccak_close(int fd, struct sockaddr *net_addr)
{
	stratum_close(fd, net_addr);
}

static uint64_t pend_block_height[MAX_COIN_IFACE];
static int is_stratum_keccak_task_pending(int *ret_iface)
{
  ...
  int ifaceIndex;
	...
	usec++;
	if (0 == (usec % KECCAK_WORK_CYCLE_TIME)) {
		usec_trigger = 1;
	}
...
static void stratum_keccak_task_gen(task_attr_t *attr)
{
  task_t *task;
  scrypt_peer peer;
...
static void stratum_keccak_timer(void)
{
  ...
  blk_iface = 0;
  is_new = is_stratum_keccak_task_pending(&blk_iface);
...
static void stratum_keccak_term(void)
{

  unet_unbind(UNET_STRATUM_KECCAK); /* close listening socket. */

}

static void stratum_keccak_accept(int fd, struct sockaddr *net_addr)
{
  sa_family_t in_fam;
	user_t *user;
  char buf[256];
...
int stratum_keccak_init(void)
{
	...
	unet_timer_set(UNET_STRATUM_KECCAK, stratum_keccak_timer); /* x1/s */
	unet_connop_set(UNET_STRATUM_KECCAK, stratum_keccak_accept);
	unet_disconnop_set(UNET_STRATUM_KECCAK, stratum_keccak_close);
