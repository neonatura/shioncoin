...
extern "C" {
	...
	int stratum_miner_submitblock(unsigned int workId, unsigned int nTime, unsigned int nNonce, char *xn_hex, char *ret_hash, double *ret_diff)
	{
		...
		//	diff = GetBitsDifficulty(pblock->nBits);
		//		if (ret_diff) 
		{
		...
		Debug("(%s) submitblock: share received (nonce: %x) (workid: %u) (pow: %s) (target: %s) (diff: %f).", iface->name, nNonce, workId, hash.ToString().substr(0,12).c_str(), hashTarget.ToString().substr(0, 12).c_str(), diff);
