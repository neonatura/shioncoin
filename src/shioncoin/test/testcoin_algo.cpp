...
/* create a block using the sha256d algorythm. */
_TEST(algo_sha256d)
{
	...
	/* set algo version. */
	block->nVersion = GetAlgoBits(ALGO_SHA256D);

	/* set difficulty for algo. */
	CBigNum diff;
	diff.SetCompact(block->nBits);
	diff /= GetAlgoWorkFactor(ALGO_SHA256D); 
	block->nBits = diff.GetCompact();

	/* generate new merkle root. */
	block->hashMerkleRoot = block->BuildMerkleTree();

	uint256 diff_target = diff.getuint256();
	//unsigned char work_target[256];
	//memcpy(work_target, (uint256 *)&diff, 32);

	uint32_t *ptarget = (uint32_t *)&diff_target;
	uint32_t hash_ar[8];
	uint256 n_hash;

	int n = 0;
	int max_nonce = 1000000000;
	do {
		block->nNonce = ++n;
		n_hash = block->GetHash();

		memcpy(hash_ar, &n_hash, 32);
		if (fulltest(hash_ar, ptarget)) {
			break;
		}
	} while (n < max_nonce);
	_TRUE(n != max_nonce);
