...
#define LTC_COIN_HEADER_SIZE SIZEOF_COINHDR_T

#define LTC_SENDHEADERS_VERSION 70012
...
static bool AlreadyHave(CIface *iface, const CInv& inv)
{
  ...
  switch (inv.type)
  {
    ...
    case MSG_WITNESS_TX:
      {
        ...
        fHave = false;
        {
          LOCK(LTCBlock::mempool.cs);
          fHave = (LTCBlock::mempool.exists(inv.hash));
        }
        if (fHave)
          return (true);
    ...
    case MSG_WITNESS_BLOCK:
      ...
      return blockIndex->count(inv.hash);// || ltc_IsOrphanBlock(inv.hash);
  }
...
bool ltc_ProcessMessage(CIface *iface, CNode* pfrom, string strCommand, CDataStream& vRecv)
{
...
//  static map<CService, CPubKey> mapReuseKey;
  ...
  if (strCommand == "version")
  {
    ...
    if (pfrom->nVersion < MIN_LTC_PROTO_VERSION)
    {
      sprintf(errbuf, "peer %s using obsolete (ltc) version %i", pfrom->addr.ToString().c_str(), pfrom->nVersion);
    ...
    if (0 != strncmp(pfrom->strSubVer.c_str(), "/LTC", 5)) {
      error(SHERR_INVAL, "(ltc) ProcessMessage: connect from wrong coin interface '%s' (%s), disconnecting\n", pfrom->addr.ToString().c_str(), pfrom->strSubVer.c_str());
    ...
    {
      nAskedForBlocks++;
      pfrom->PushGetBlocks(GetBestBlockIndex(LTC_COIN_IFACE), uint256(0));
    }
#endif
    ...
    Debug("received LTC version message: version %d, blocks=%d, us=%s, them=%s, peer=%s\n", pfrom->nVersion, pfrom->nStartingHeight, addrMe.ToString().c_str(), addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str());
  ...
  {
    pfrom->vRecv.SetVersion(min(pfrom->nVersion, LTC_PROTOCOL_VERSION));

#if 0
    if (pfrom->nVersion >= LTC_SENDHEADERS_VERSION) {
...
//        ServiceBlockEventUpdate(LTC_COIN_IFACE);
    ...
    BOOST_FOREACH(const CInv& inv, vInv)
    {
      ...
      if (inv.type == MSG_BLOCK ||
        ...
        CBlockIndex *pindex = GetBlockIndexByHash(ifaceIndex, inv.hash);
				if (pindex) {
					...
					if (block.ReadFromDisk(pindex) && block.CheckBlock() &&
							pindex->nHeight <= GetBestHeight(LTC_COIN_IFACE)) {
						if (inv.type == MSG_BLOCK) {
              ...
              if (filter) {
                ...
                BOOST_FOREACH(PairType& pair, merkleBlock.vMatchedTxn)
                  if (!pfrom->setInventoryKnown.count(CInv(LTC_COIN_IFACE, MSG_TX, pair.second)))
                    pfrom->PushMessage("tx", block.vtx[pair.first]);
              }
            } 
          } else {
            Debug("LTC: WARN: failure validating requested block '%s' at height %d\n", block.GetHash().GetHex().c_str(), pindex->nHeight);
            block.print();
          }
        ...
        if (pool->GetTx(inv.hash, tx)) {
          ...
          Debug("(ltc) PushTx: tx \"%s\" to peer \"%s\".", inv.hash.GetHex().c_str(), pfrom->addrName.c_str());
        }
        ...
        if (pool->GetTx(inv.hash, tx)) {
          pfrom->PushTx(tx);
          Debug("(ltc) PushTx: wit-tx \"%s\" to peer \"%s\".", inv.hash.GetHex().c_str(), pfrom->addrName.c_str());
        }
      }
  ...
  else if (strCommand == "tx")
  {
    ...
    if (tx.AcceptToMemoryPool(txdb, true, &fMissingInputs)) {
      ...
      for (unsigned int i = 0; i < vWorkQueue.size(); i++)
      {
        ...
        for (map<uint256, CDataStream*>::iterator mi = LTC_mapOrphanTransactionsByPrev[hashPrev].begin();
            mi != LTC_mapOrphanTransactionsByPrev[hashPrev].end();
            ++mi)
        {
    ...
    {
      ltc_AddOrphanTx(vMsg);

      // DoS prevention: do not allow LTC_mapOrphanTransactions to grow unbounded
      unsigned int nEvicted = ltc_LimitOrphanTxSize(MAX_ORPHAN_TRANSACTIONS(iface));
      if (nEvicted > 0)
  ...
  else if (strCommand == "block")
  {
    ...
    Debug("(ltc) ProcessBlock: received block \"%s\" from \"%s\".", hash.GetHex().c_str(), pfrom->addr.ToString().c_str());
  ...
  else if (strCommand == "getaddr")
  {
    ...
    if (!pfrom->fInbound) {
      error(SHERR_ACCESS, "(ltc) warning: Outgoing connection requested address list.");
      return true;
    }
  ...
  else if (strCommand == "mempool")
  {
    ...
    LOCK2(LTCBlock::mempool.cs, pfrom->cs_filter);
    LTCBlock::mempool.queryHashes(vtxid);
    vector<CInv> vInv;
    CBloomFilter *filter = pfrom->GetBloomFilter();
    BOOST_FOREACH(uint256& hash, vtxid) {
      CInv inv(LTC_COIN_IFACE, MSG_TX, hash);
      if ((filter && filter->IsRelevantAndUpdate(LTCBlock::mempool.lookup(hash), hash)) ||
          (!filter))
        vInv.push_back(inv);
  ...
  {
    CBloomFilter filter(LTC_COIN_IFACE);
    vRecv >> filter;
  ...
  else if (strCommand == "filterclear")
  {
    ...
    Debug("ltc_ProcessBlock: received 'sendcmpct'");
    ...
    Debug("ltc_ProcessBlock: received 'getblocktxn'");
  }
    ...
    Debug("(ltc) ProcessMessage[feefilter]: info: min fee %f", (double)newFeeFilter/COIN); 
#if 0

    if (MoneyRange(iface, newFeeFilter) &&
        newFeeFilter >= LTC_MIN_TX_FEE &&
        newFeeFilter < LTC_MAX_TX_FEE) {
    ...
    for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
    {
      ...
      if (inv.type == MSG_TX || inv.type == MSG_WITNESS_TX) {
        Debug("ltc_ProcessMessage: received notfound message: tx '%s'.", inv.hash.GetHex().c_str());
      }
...
/* .. */
  ...
  else
  {
   ...
   Debug("ltc_ProcessMessage: unknown cmd '%s'\n", strCommand.c_str()); 
  }
...
bool ltc_ProcessMessages(CIface *iface, CNode* pfrom)
{
  ...
  loop
  {
    ...
    int nHeaderSize = LTC_COIN_HEADER_SIZE;
    if (vRecv.end() - pstart < nHeaderSize)
    {
      if ((int)vRecv.size() > nHeaderSize)
      {
        Debug("(ltc) warning: PROCESSMESSAGE MESSAGESTART NOT FOUND");
    ...
    if (pstart - vRecv.begin() > 0)
      Debug("(ltc) warning: PROCESSMESSAGE SKIPPED %d BYTES", pstart - vRecv.begin());
...
//fprintf(stderr, "DEBUG: LTC: PROCESSMESSAGE: ERRORS IN HEADER %s\n", hdr.GetCommand().c_str());
    ...
    {
      error(SHERR_2BIG, "ltc_ProcessMessages(%s, %u bytes) : nMessageSize > MAX_SIZE\n", strCommand.c_str(), nMessageSize);
      continue;
    }
    ...
    {
//"ltc_ProcessMessages(%s, msg is %u bytes, buff is %u bytes) : CHECKSUM ERROR nChecksum=%08x hdr.nChecksum=%08x buff nVersion=%u\n", strCommand.c_str(), nMessageSize, (unsigned int)vRecv.size(), nChecksum, hdr.nChecksum, (unsigned int)vRecv.nVersion);
      continue;
    }
    ...
    try
    {
      {
        LOCK(cs_main);
        fRet = ltc_ProcessMessage(iface, pfrom, strCommand, vMsg);
      }
      if (fShutdown)
        return true;
    }
...
bool ltc_SendMessages(CIface *iface, CNode* pto, bool fSendTrickle)
{
  ...
  if (lockMain) {
    ...
    if (!IsInitialBlockDownload(LTC_COIN_IFACE) && (GetTime() - nLastRebroadcast > 24 * 60 * 60))
    {
      {
        LOCK(cs_vNodes);
        BOOST_FOREACH(CNode* pnode, vNodes)
