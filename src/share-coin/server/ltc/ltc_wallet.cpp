...
bool ltc_LoadWallet(void)
{
  ...
  LTC_COINBASE_FLAGS << std::vector<unsigned char>(pszP2SH, pszP2SH+strlen(pszP2SH));
  ...
  if (fFirstRun)
  {
    ...
    if (!ltcWallet->GetKeyFromPool(newDefaultKey, false))
      strErrors << _("Cannot initialize keypool") << "\n";
    ltcWallet->SetDefaultKey(newDefaultKey);
    if (!ltcWallet->SetAddressBookName(ltcWallet->vchDefaultKey.GetID(), ""))
...
void LTCWallet::RelayWalletTransaction(CWalletTx& wtx)
{
  LTCTxDB txdb;
...
void LTCWallet::ResendWalletTransactions()
{
  CIface *iface = GetCoinByIndex(LTC_COIN_IFACE);
  ...
  if (LTCBlock::nTimeBestReceived < nLastTime)
    return;
  nLastTime = GetTime();
  ...
  // Rebroadcast any of our txes that aren't in a block yet
  {
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
    {
      ...
      if (LTCBlock::nTimeBestReceived - (int64)wtx.nTimeReceived > 5 * 60)
...
void LTCWallet::ReacceptWalletTransactions()
{
	{
		LOCK(cs_wallet);
...
bool LTCWallet::CommitTransaction(CWalletTx& wtxNew)
{
  CIface *iface = GetCoinByIndex(LTC_COIN_IFACE);
  CTxMemPool *pool = GetTxMemPool(iface);
...
bool LTCWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
  CIface *iface = GetCoinByIndex(LTC_COIN_IFACE);
  int64 nValue = 0;
...

  {
    LOCK2(cs_main, cs_wallet);
    {
      ...
      loop
      {
        ...
        if (nFeeRet < LTC_MIN_TX_FEE && nChange > 0 && nChange < CENT)
        {
          int64 nMoveToFee = min(nChange, LTC_MIN_TX_FEE - nFeeRet);
          nChange -= nMoveToFee;
          nFeeRet += nMoveToFee;
        }
        ...
        unsigned int nIn = 0;
        BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins) {
          CSignature sig(LTC_COIN_IFACE, &wtxNew, nIn);
        ...
        if (AllowFree(dPriority) && nBytes < 1000)
          fAllowFree = true;
        int64 nMinFee = wtxNew.GetMinFee(LTC_COIN_IFACE, 1, fAllowFree, GMF_SEND);
#endif
...
bool LTCWallet::CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
...
void LTCWallet::AddSupportingTransactions(CWalletTx& wtx)
{
  wtx.AddSupportingTransactions();
}

#ifdef USE_LEVELDB_COINDB
bool LTCWallet::UnacceptWalletTransaction(const CTransaction& tx)
{
  CIface *iface = GetCoinByIndex(LTC_COIN_IFACE);
...
bool LTCWallet::CreateAccountTransaction(string strFromAccount, const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, string& strError, int64& nFeeRet)
{
  CIface *iface = GetCoinByIndex(LTC_COIN_IFACE);
...

  {
    LOCK2(cs_main, cs_wallet);
    {
      ...
      loop
      {
        ...
        int64 nChange = nValueIn - nValue - nFeeRet;
        // if sub-cent change is required, the fee must be raised to at least LTC_MIN_TX_FEE
        // or until nChange becomes zero
        // NOTE: this depends on the exact behaviour of GetMinFee
        if (nFeeRet < LTC_MIN_TX_FEE && nChange > 0 && nChange < CENT)
        {
...
bool LTCWallet::CreateAccountTransaction(string strFromAccount, CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, string& strError, int64& nFeeRet)
{
  vector< pair<CScript, int64> > vecSend;
  vecSend.push_back(make_pair(scriptPubKey, nValue));
...
unsigned int LTCWallet::GetTransactionWeight(const CTransaction& tx)
{

  unsigned int nBytes;

  nBytes = 
    ::GetSerializeSize(tx, SER_NETWORK, LTC_PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) * (LTC_WITNESS_SCALE_FACTOR - 1) +
    ::GetSerializeSize(tx, SER_NETWORK, LTC_PROTOCOL_VERSION);
...
#if 0
bool AllowFree() {
  return dPriority > ltc_AllowFreeThreshold();
}
#endif
